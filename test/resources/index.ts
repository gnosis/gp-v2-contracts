...
export const solveTestCase = function (testCaseInput: TestCaseInput, debug = false): Solution {
  ...
  if (sumSellDemandToken0.isZero() || sumSellAmountToken1.isZero()) {
    ...
    return ({clearingPrice: {numerator: BigNumber.from('0'), denominator: BigNumber.from('0')},
      sellOrdersToken0: [],
      sellOrdersToken1: []
    });
  }
  const highestSellOrderToken0 = testCaseInput.sellOrdersToken0[testCaseInput.sellOrdersToken0.length - 1];
  const highestSellOrderToken1 = testCaseInput.sellOrdersToken1[testCaseInput.sellOrdersToken1.length - 1];
  ...
  log('Highest bid selling OrderToken0', highestSellOrderToken0.sellAmount.toString());
  log('Highest bid selling OrderToken1', highestSellOrderToken0.buyAmount.toString());
  if (sumSellDemandToken0.mul(highestSellOrderToken1.sellAmount)
    .lt(sumSellAmountToken1.mul(highestSellOrderToken1.buyAmount))) {
    log('switching token0 and token1');

    return solveTestCase({fundingAMMToken0: testCaseInput.fundingAMMToken1,
  ...
  if (highestSellOrderToken0.sellAmount.mul(clearingPrice.denominator)
    ...
    return solveTestCase({fundingAMMToken0: testCaseInput.fundingAMMToken0,
