...
- `GPv2AllowanceManager.sol`: The allowance manager is the contract users approve an ERC20 allowance for in order to interact with the protocol; it is intentionally a kept as a separate contracts so that SC interactions do not have access to user allowances.
- `interfaces`
